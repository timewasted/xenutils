...
# hosts, and eventually reboot the Xen pool master. 
#
...
# Log file location
log_file="/var/log/xen-reboot.log"
...
function reboot_xenhosts () {
	...
	for uuid in ${xen_slave_uuids[@]}; do
		...
		sleep 1
		log_date "Rebooting slave host $slave_name (UUID: $uuid)"
		xe host-reboot uuid=$uuid &
		sleep 1
	done
	...
	for uuid in ${xen_slave_uuids[@]}; do
		if [ $(( SECONDS - start_time )) -lt $xen_slave_timeout ]; then
			while true; do
				...
				if [ $? -eq 0 ]; then
					log_date "Not all slave hosts rebooted, continuing to wait..."
					sleep 10
					continue
				else
					sleep 2
			...
			log_date "Slave host reboot timeout, proceeding with master host reboot"
		fi
	done
	...
	sleep 1
	log_date "Rebooting master host $xen_master_name (UUID: $xen_master_uuid)"
	xe host-reboot uuid=$xen_master_uuid
	sleep 1
}
...
function main () {
	log_date "==============================================================================="
	log_date "Received reboot request, initiating reboot procedure"
	log_date "==============================================================================="
